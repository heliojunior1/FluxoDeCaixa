...
from ..utils.constants import MONTH_NAME_PT, DAY_ABBR_PT, MONTH_ABBR_PT
...
def get_resumo_data(ano_selecionado, meses_selecionados, estrategia, cenario_selecionado_id):
...
def get_indicadores_data(ano_selecionado, meses_selecionados, tipo_selecionado):
    meses_nomes = {i: calendar.month_name[i].capitalize() for i in range(1, 13)}
    tipo_entrada = TipoLancamento.query.filter_by(dsc_tipo_lancamento="Entrada").first()
    tipo_saida = TipoLancamento.query.filter_by(dsc_tipo_lancamento="Saída").first()
    id_entrada = tipo_entrada.cod_tipo_lancamento if tipo_entrada else -1
...
def get_analise_comparativa_data(ano1, ano2, meses_selecionados, tipo_analise):
    meses_nomes = {i: calendar.month_name[i].capitalize() for i in range(1, 13)}
    data = {}
...
def get_saldos_diarios_data(data_ref):
...
def get_dfc_data(periodo, ano_selecionado, mes_selecionado, meses_selecionados, estrategia, cenario_selecionado_id):
    hoje = date.today()
...
def get_dfc_eventos(seq, periodo, col, mes_ano, estrategia, cenario_id):
